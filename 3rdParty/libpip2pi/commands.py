...
try:
    cmnd = "pip --version"
    pip_pkg_info = subprocess.check_output(cmnd, stderr=subprocess.STDOUT).decode("utf-8")
    pip_version = tuple(try_int(x) for x in pip_pkg_info.split(" ")[1].split("."))
    has_pip = True
except subprocess.CalledProcessError as error_msg:
    log_error("Fatal error when trying to get pip version\n{}".format(error_msg))
    pip_version = None
    has_pip = False
